...
def get_coarse_labels():
    root='E:\\PycharmProjects\\CFBSNet\\main\\datasets\\imbalance_cifar10\\cifar-100-python\\train'
...
class IMBALANCECIFAR10(torchvision.datasets.CIFAR10):
    ...
    def get_coarse_fine(self):
        coarse=self.coarse
        fine=self.targets
        coarse_fine=np.vstack((fine,coarse)).T
        return coarse_fine
    def get_coarse(self):
        coarse_annos = []
        for coarse in self.coarse:
            coarse_annos.append({'coarse_id':int(coarse)})
        return coarse_annos
    ...
    def gen_imbalanced_data(self, img_num_per_cls):
        ...
        coarse_labels = get_coarse_labels()
        coarse_targets = np.array(coarse_labels, dtype=np.int64)
        ...
        for the_class, the_img_num in zip(classes, img_num_per_cls):
            ...
            coarse = [coarse_targets[i] for i in selec_idx]
            new_coarse.extend(coarse)
