...
from CFBSNet.lib.core.evaluate import accuracy, AverageMeter, FusionMatrix
    ...
    **kwargs
    ...
    for i, (image, label, meta) in enumerate(trainLoader):
        ...
        for i in range(len(label)):
            fl = int(label[i])
            cl = coarse_idx[fl]
            coarse_label.append(cl)

        coarse_label=np.array(coarse_label)
        coarse_label=torch.tensor(np.array(coarse_label))

        loss, now_acc = combiner.forward(model, criterion, image, label, meta, coarse_label)
